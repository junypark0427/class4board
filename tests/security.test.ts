...
test('server submission creates private unread opinion; cooldown is transactional', async () => {
  ...
  const result = await db.query<{id:string;status:string}>('select * from public.posts');
  assert.equal(result.rows.length,1); assert.equal(result.rows[0].status,'unread'); postId=result.rows[0].id;
});
...
test('ordinary authenticated users have no admin access or self-promotion', async () => {
  ...
  await assert.rejects(db.query('select public.moderate_post($1,1,$2,false,$3,$4)',[postId,'completed','','']), /not_authorized/);
});
test('both admins have equal access; notes are private and changes are attributed', async () => {
  await asRole('authenticated',president);
  await db.query('select public.moderate_post($1,1,$2,false,$3,$4)',[postId,'reviewed','비공개 관리자 메모','확인했어요.']);
  ...
  await db.query('select public.moderate_post($1,2,$2,false,$3,$4)',[postId,'forwarded','전달 완료 메모','선생님께 전달했어요.']);
...
test('stale saves are rejected and hide/unhide is audited without changing workflow', async () => {
  await asRole('authenticated',president);
  await assert.rejects(db.query('select public.moderate_post($1,1,$2,false,$3,$4)',[postId,'completed','','']), /conflict_reload/);
  await db.query('select public.moderate_post($1,3,$2,true,$3,$4)',[postId,'forwarded','전달 완료 메모','선생님께 전달했어요.']);
  assert.equal((await db.query<{hidden:boolean}>('select hidden from public.posts')).rows[0].hidden,true);
  await db.query('select public.moderate_post($1,4,$2,false,$3,$4)',[postId,'completed','전달 완료 메모','처리했어요.']);
});
...
test('database length checks roll back throttle and reject invalid states', async () => {
  ...
  await asRole('authenticated',deputy);
  await assert.rejects(db.query('select public.moderate_post($1,5,$2,false,$3,$4)',[postId,'published','','']), /check constraint/);
});
