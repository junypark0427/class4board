...
function Editor({ post, onClose, onSaved }: { post:Post; onClose:()=>void; onSaved:()=>Promise<void> }) {
  ...
  async function save(e:FormEvent) {
    ...
    try { const {error}=await browserDatabase()!.rpc('moderate_post',{p_id:post.id,p_version:post.version,p_status:status,p_hidden:hidden,p_admin_note:note,p_reply:reply});
  ...
  return <section className="form-panel editor" aria-label="의견 상세"><div className="post-meta"><span className="tag">{post.category}</span><button className="text-button" onClick={onClose}>닫기 ✕</button></div><h2>{post.title}</h2><p className="subtle">{new Date(post.created_at).toLocaleString('ko-KR',{timeZone:'Asia/Seoul'})} · 익명</p><p className="prose opinion-content">{post.content}</p><div className="request-badges"><span>{post.teacher_requested?'선생님 전달 희망':'선생님 전달 요청 없음'}</span><span>{post.reply_requested?'결과 확인 희망':'결과 확인 요청 없음'}</span></div><form onSubmit={save}><label className="field">처리 상태<select value={status} onChange={e=>setStatus(e.target.value as Post['status'])}>{Object.entries(statusLabels).map(([k,v])=><option key={k} value={k}>{v}</option>)}</select></label>{status==='forwarded' && <p className="notice">선생님께 실제로 전달했을 때 선택해주세요. 자동으로 전송되지는 않아요.</p>}<label className="field">관리자 메모 <span className="field-hint">반장·부반장만 볼 수 있어요</span><textarea rows={4} maxLength={4000} value={note} onChange={e=>setNote(e.target.value)} placeholder="확인한 내용, 선생님께 전달할 요점 등을 기록해주세요."/></label>{post.reply_requested && <label className="field">작성자에게 전할 답변 <span className="field-hint">비밀 확인번호로 볼 수 있어요. 개인정보는 적지 마세요.</span><textarea rows={4} maxLength={2000} value={reply} onChange={e=>setReply(e.target.value)} placeholder="어떻게 처리했는지 알려주세요."/></label>}<label className="check"><input type="checkbox" checked={hidden} onChange={e=>setHidden(e.target.checked)}/><span>이 의견 숨기기<small>기본 목록에서 제외돼요. 숨긴 의견 필터로 다시 찾을 수 있어요.</small></span></label>{message && <p className="error" role="alert">{message}</p>}<button className="button primary full" disabled={busy}>{busy?'저장 중…':'변경사항 저장'}</button></form><div className="audit"><h3>관리자 작업 기록</h3><p className="helper">최근 30건 · 메모와 답변의 본문은 기록에 복사하지 않아요.</p>{auditError ? <p className="error">기록을 불러오지 못했어요.</p> : !actions.length ? <p className="subtle">아직 변경 기록이 없어요.</p> : <ul>{actions.map(a=><li key={a.id}><strong>{a.actor_name}</strong><span>{a.changes.status && `${statusLabels[a.changes.status[0]]} → ${statusLabels[a.changes.status[1]]}`}{a.changes.hidden!==undefined && (a.changes.hidden?' · 숨김':' · 숨김 해제')}{a.changes.admin_note_changed && ' · 메모 수정'}{a.changes.reply_changed && ' · 답변 수정'}</span><time>{new Date(a.created_at).toLocaleString('ko-KR',{timeZone:'Asia/Seoul'})}</time></li>)}</ul>}</div></section>;
}
